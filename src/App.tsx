...
import { GeoJSON, MapContainer, Marker, Tooltip, useMap, useMapEvents } from 'react-leaflet'
...
function spotMarkerIcon(level: SurfLevel, active: boolean) {
  const size = markerRadius(level, active) * 2
  return divIcon({
    className: 'spot-marker-wrapper',
    html: `<div class="spot-marker${active ? ' is-active' : ''}" style="width:${size}px;height:${size}px;background:${markerColor(level)}"></div>`,
    iconSize: [size, size],
      ...
      <main className="dashboard">
        <section className="map-column">
          <div className="section-card map-card">
            ...
            {dataStatus !== 'ready' ? <p className={`status-note ${dataStatus}`}>{dataMessage}</p> : null}
                ...
                {markerNodes.map((node) => {
                  if (node.kind === 'spot') {
                    ...
                    return (
                      <Marker
                        key={node.spot.id}
                        position={[node.spot.lat, node.spot.lng]}
                        icon={spotMarkerIcon(node.spot.currentLevel, active)}
                        eventHandlers={{
